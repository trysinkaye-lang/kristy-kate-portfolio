          ...
          {portfolioTab === "projects" ? (
            <div className="mt-12">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {softwareProjects.map((item, index) => (
                  <button key={item.slug} type="button" onClick={() => goTo(index)} className={`rounded-2xl border p-5 text-left transition ${activeProject === index ? "border-white/20 bg-white/[.055]" : "border-white/[.06] bg-transparent hover:bg-white/[.025]"}`}>
                    <span className="text-xs text-zinc-600">0{index + 1}</span>
