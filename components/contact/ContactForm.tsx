...
export function ContactForm({ configured, initialSubject = "" }: { configured: boolean; initialSubject?: string }) {
  ...
  return <form className={styles.form} onSubmit={submit} onChange={() => { submissionId.current = null; }} onFocus={() => { if (!started.current) { started.current = true; trackPortfolioEvent("contact_start", { source: "form" }); } }} aria-label="Contact Kristy Kate" aria-describedby="form-note">
    ...
    <div role="status" aria-live="polite" aria-atomic="true" className={styles.result} data-success={result?.ok}>{result?.message}</div>
