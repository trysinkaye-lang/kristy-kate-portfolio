...
function ThemeToggle() {
  ...
  return <button type="button" className={styles.theme} aria-label={dark ? "Use light mode" : "Use dark mode"} onClick={() => setTheme(dark ? "light" : "dark")}><span aria-hidden="true">{dark ? "◑" : "◐"}</span><span className="sr-only">Color theme</span></button>;
}
export function Nav() {
  ...
  return <header className={styles.header}>
    ...
    <dialog ref={dialog} id="mobile-navigation" className={styles.dialog} aria-label="Navigation" onCancel={close} onClose={() => setOpen(false)} onKeyDown={event => {
      ...
      <nav aria-label="Mobile navigation">{links.map(([label, href]) => <Link href={href} key={href} onClick={close}>{label}</Link>)}</nav>
