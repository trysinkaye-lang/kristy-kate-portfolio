...
test("work-first hero, real project order, and primary interactions", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(hero.getByRole("navigation", { name: "Featured projects" })).toBeVisible();
  for (const project of ["RBIM", "C.O. Designs", "AHDIS", "Marci Metzger"]) {
    await expect(hero.getByRole("navigation", { name: "Featured projects" }).getByText(project, { exact: true })).toBeVisible();
  }
