...
const routes = ["/", "/projects", "/about", "/contact", ...["rbim","co-designs","ahdis","marci-metzger","lacomus","erp-system","design-systems"].map(slug => `/projects/${slug}`)];
...
test("every route avoids overflow at mobile, tablet, and desktop", async ({ page }) => {
...
test("SEO covers canonical URLs, social previews, all projects, and not-found", async ({ page, request }) => {
  for (const route of ["/", "/about", "/contact", "/projects/rbim", "/projects/lacomus"]) {
    await page.goto(route);
  ...
  for (const route of routes.slice(4)) expect(sitemap).toContain(route);
